...
def test_missing_version_file():
    ...
    assert CAN.__version__ != "UNKNOWN"
    os.rename("../pycanlib/version.txt", "../pycanlib/version.txt.old")
    CAN = reload(CAN)
    os.rename("../pycanlib/version.txt.old", "../pycanlib/version.txt")
    assert CAN.__version__ == "UNKNOWN"

"""
def test_check_status():
    _channels = [-2, -1, 0, 1]
    _valid = [False, False, True, True]
    for (_channel, _validity) in zip(_channels, _valid):
        testLogger.debug(zip(_channels, _valid))
        try:
            _handle = canlib.canOpenChannel(_channel, canlib.canOPEN_ACCEPT_VIRTUAL)
        except canlib.CANLIBError:
            testLogger.debug("Exception thrown by CANLIB", exc_info=True)
            _handle = -1
        yield call_check_status, _channel, _handle, _validity


def call_check_status(channel, handle, valid):
    _tmr_val = None
    try:
        _tmr_val = canlib.canReadTimer(handle)
    except canlib.CANLIBError:
        testLogger.debug("Exception thrown by CANLIB", exc_info=True)
    testLogger.debug(type(_tmr_val))
    if valid:
        assert _tmr_val > 0
        canlib.canClose(handle)
    else:
        assert _tmr_val == None
"""

def test_good_xml_object():
    _log_message = CAN.LogMessage()
    _log_message2 = CAN.LogMessage(xml=_log_message.to_xml())
    assert _log_message.to_xml().toprettyxml() == \
      _log_message2.to_xml().toprettyxml()
    _can_message = CAN.Message()
    _can_message2 = CAN.Message(timestamp=1.2345)
    _msg_list = CAN.MessageList(messages=[_can_message, _can_message2])
    _msg_list2 = CAN.MessageList(xml=_msg_list.to_xml())
    assert _msg_list.to_xml().toprettyxml() == \
      _msg_list.to_xml().toprettyxml()
    _can_message2_xml = _can_message2.to_xml()
    _can_message2_xml.childNodes.append(CAN.Message().to_xml())
    _can_message3 = CAN.Message(xml=_can_message2_xml)
    assert _can_message2.to_xml().toprettyxml() == \
      _can_message3.to_xml().toprettyxml()

def test_incorrect_xml_object():
    _can_message = CAN.Message()
    _log_message = None
    try:
        _log_message = CAN.LogMessage(xml=_can_message.to_xml())
    except CAN.InvalidMessageParameterError:
        testLogger.debug("Exception thrown by pycanlib", exc_info=True)
    assert _log_message == None


def test_good_info_message():
    _info_message = CAN.InfoMessage(timestamp=12345, info="info message")
    _info_message2 = CAN.InfoMessage(xml=_info_message.to_xml())
    assert _info_message.to_xml().toprettyxml() == \
      _info_message2.to_xml().toprettyxml()
