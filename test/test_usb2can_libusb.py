...
from can.interfaces.usb2can_libusb.usb2can_libusb_bus import message_convert_rx, message_convert_tx, Usb2CanLibUsbBus
from can.interfaces.usb2can_libusb.can_8dev_usb_utils import Can8DevRxFrame, Can8DevCommandFrame, Can8DevCommand, can_8dev_open_frame
...
class TestUsb2CanLibUsbBus(unittest.TestCase):
    def test_receive_deserialize(self) -> None:
        ...
        self.assertEqual(recv_msg.data, bytes([0x4e, 0xf1, 0xff, 0x1f, 0x00, 0x00, 0x7E, 0x00]))
        self.assertEqual(recv_msg.timestamp, 8607.200000)

    def test_transmit_serialize(self) -> None:
        send_packet = Message(arbitration_id=0xC0FFEE, is_extended_id=True,data=[0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08])
    ...
    def test_open_command(self) -> None:
        ...
        listenonly = False
        oneshot = False
        open_command = can_8dev_open_frame(phase_seg1, phase_seg2, sjw, brp, loopback, listenonly, oneshot)
        self.assertEqual(open_command.to_bytes().hex(), '11000209000601010008000000020022')
