...
class LogCAN(ipipe.Display):
    ...
    def display(self):
        ...
        CAN.Log(log_info=CAN.LogInfo(log_start_time=self.__start_time,
                                     ...
                                     original_file_name=("LogCAN_%s.dat" % self.__start_time.strftime("%Y%m%d_%H%M%S")),
                                     tester_name=os.getenv("USERNAME")),
                channel_info=None,
                machine_info=CAN.get_host_machine_info(),
                message_lists=[CAN.MessageList(messages=self.msglist)]).write_to_file(path=os.path.expanduser(os.path.join("~", "LogCAN")))
