...
class IXXATBus(BusABC):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if isinstance(timing, BitTimingFd):
            # if a BitTimingFd instance has been passed, we can presume the user wants to use FD capability
            fd = True
        ...
        if not timing:
            # only use bitrate and the deprecated args if no timing argument is provided
            if tseg1_abr and tseg2_abr and sjw_abr:
                if not tseg1_dbr and not tseg2_dbr and not sjw_dbr:
                    # if only the arbitration rate segments have been specified, duplicate them for data rate segments
                    tseg1_dbr = tseg1_abr
                    tseg1_dbr = tseg2_dbr
                    sjw_dbr = sjw_abr
                try:
                    ...
                    raise CanInitializationError(
                        "Could not initialise the channel with the given bitrate and segment timings"
                    ) from exc
            else:
                try:
                    ...
                    raise CanInitializationError(
                        "Could not initialise the channel with the given bitrate and sample point target of 80%."
                    ) from exc
        elif isinstance(timing, BitTiming):
            # if a standard BitTiming class has been passed, convert it to a BitTimingFD instance
            timing = BitTimingFd.from_bitrate_and_segments(
                f_clock=timing.f_clock,
                nom_bitrate=timing.bitrate,
                nom_tseg1=timing.tseg1,
                nom_tseg2=timing.tseg2,
                nom_sjw=timing.sjw,
                data_bitrate=timing.bitrate,
                data_tseg1=timing.tseg1,
                data_tseg2=timing.tseg2,
                data_sjw=timing.sjw,
            )
    ...
    @staticmethod  # TODO - implement BitTiming class
