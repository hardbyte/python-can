...
class ZCAN_TX_TIMEOUT(Structure):  # Tx Timeout
...
class ZCAN_20_MSG(Structure):
    _fields_ = [("header", ZCAN_MSG_HDR), ("dat", c_ubyte * 8)]


class ZCAN_FD_MSG(Structure):
    _fields_ = [("header", ZCAN_MSG_HDR), ("dat", c_ubyte * 64)]
...
def vci_channel_open(
    dt, di, ch, clock, atiming, dtiming=None, mode=ZCAN_MODE.NORMAL
) -> bool:
    init = ZCANFD_INIT()
...
def vci_channel_enable_tres(dt, di, ch, value) -> bool:
    ...
    ret = lib.VCI_SetReference(
        c_uint32(dt), c_uint32(di), c_uint32(ch), c_uint32(ref), byref(tres)
    )
    if ret == 0:
        log.error(
            f'CH{ch}: Failed to {"enable" if value else "disable"} '
            f"termination resistor"
        )
    else:
        log.info(
            f'CH{ch}: {"Enable" if value else "Disable"} '
...
def vci_channel_set_tx_timeout(dt, di, ch, value) -> bool:
    ...
    ret = lib.VCI_SetReference(
        c_uint32(dt), c_uint32(di), c_uint32(ch), c_uint32(ref), byref(timeout)
    )
    if ret == 0:
        log.error(f"CH{ch}: Failed to set tx timout {value}ms!")
    else:
        log.debug(f"CH{ch}: Set tx timout to {value}ms")
    return ret != 0


def vci_can_send(dt, di, ch, msgs, count) -> int:
    ret = lib.VCI_Transmit(
        c_uint32(dt), c_uint32(di), c_uint32(ch), byref(msgs), c_uint32(count)
    )
    if ret == 0:
        log.error(f"CH{ch}: Failed to send CAN message(s)")
    else:
        log.debug(f"CH{ch}: Sent {ret} CAN message(s)")
    return ret


def vci_canfd_send(dt, di, ch, msgs, count) -> int:
    ret = lib.VCI_TransmitFD(
        c_uint32(dt), c_uint32(di), c_uint32(ch), byref(msgs), c_uint32(count)
    )
    if ret == 0:
        log.error(f"CH{ch}: Failed to send CAN-FD message(s)")
    else:
        log.debug(f"CH{ch}: Sent {ret} CAN-FD message(s)")
    return ret
...
def vci_can_get_recv_num(dt, di, ch) -> int:
    ...
    log.debug(f"CH{ch}: {ret} message(s) in CAN FIFO")
    return ret
...
def vci_canfd_get_recv_num(dt, di, ch) -> int:
    ch = ch | 0x8000_0000
    ret = lib.VCI_GetReceiveNum(c_uint32(dt), c_uint32(di), c_uint32(ch))
    log.debug(f"CH{ch&0x7FFF_FFFF}: {ret} message(s) in CAN-FD FIFO")
    return ret
