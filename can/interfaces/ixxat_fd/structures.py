...
class CANMSGINFO(ctypes.Union):
    class Bytes(ctypes.Structure):
        _fields_ = [
            ...
            ("bAddFlags", ctypes.c_uint8), # extended flags (see CAN_MSGFLAGS2_ constants)
    ...
    class Bits(ctypes.Structure):
        _fields_ = [
            ...
            ("ext", ctypes.c_uint32, 1),   # extended frame format (0=standard, 1=extended)
...
class CANCAPABILITIES2(ctypes.Structure):
    _fields_ = [
        ...
        ("dwFeatures", ctypes.c_uint32),     # supported features (see CAN_FEATURE_ constants)
        ...
        ("dwTscClkFreq" ,  ctypes.c_uint32), # clock frequency of the time stamp counter [Hz]
        ("dwTscDivisor" ,  ctypes.c_uint32), # divisor for the message time stamp counter
        ("dwCmsClkFreq" ,  ctypes.c_uint32), # clock frequency of cyclic message scheduler [Hz]
        ("dwCmsDivisor" ,  ctypes.c_uint32), # divisor for the cyclic message scheduler
        ("dwCmsMaxTicks",  ctypes.c_uint32), # maximum tick count value of the cyclic message
        ("dwDtxClkFreq" ,  ctypes.c_uint32), # clock frequency of the delayed message transmitter [Hz]
        ("dwDtxDivisor" ,  ctypes.c_uint32), # divisor for the delayed message transmitter
        ("dwDtxMaxTicks",  ctypes.c_uint32), # maximum tick count value of the delayed message transmitter
    ]
...
class CANCYCLICTXMSG2(ctypes.Structure):
    _fields_ = [
        ...
        ("bIncrMode", ctypes.c_uint8),   # auto increment mode (see CAN_CTXMSG_INC_ const)
        ("bByteIndex", ctypes.c_uint8),  # index of the byte within abData[] to increment
