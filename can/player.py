...
def _parse_loop(value: str) -> Union[int, float]:
...
def _format_player_start_message(iteration: int, loop_count: Union[int, float]) -> str:
    """
...
def main() -> None:
    ...
    with create_bus_from_namespace(results) as bus:
        loop_count: Union[int, float] = results.loop
        iteration = 0
        try:
